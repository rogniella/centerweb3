...
<div class="panel panel-info">   
    ...
    <div class="input-group">
      <select id="producto1" class="form-control"></select>
      <span >
      <button onclick="forzarBusqueda()">🔍</button>
      </span>
    </div>
...
$producto.select2({
      ...
      ajax: {
        url:   '../productos/buscaproducto?familia='+$('#id_familia').val() +'&',
        dataType: 'json',
        global: false,
        delay: 250,
        data: function (params) {
          return {
            terms: params.term // lo que el usuario escribe
          };
        },
        processResults: function (data) {
          // Se espera que el backend devuelva: [{id:1, nombre:"Manzana"}, ...]
          return {
 ...
 // Función para abrir el dropdown si hay texto suficiente
 function forzarBusqueda() {
    
    console.log("Busca Producto :" );
    var searchField = $producto.data('select2').$container.find('input.select2-search__field');
    var searchTerm = searchField.val() || '';
    console.log("Busca Producto :", searchField.val() , searchTerm.length );
...
// Botón "Buscar"
$('#btnBuscar').on('click', forzarBusqueda);

// Tecla Enter
$producto.on('keypress', function (e) {
    console.log("key Producto :", e.which );
  ...
  if (e.which === 13) {
    ...
    // Evento opcional: después de seleccionar
    $producto.on('select2:select', function (e) {

      var data = e.params.data;
      console.log("Producto seleccionado:", data.id, data.text);

      // Puedes llenar un campo oculto, o hacer algo con el ID
      $('#id_producto').val(data.id);
                    $('#descrip_producto').val(data.text);
                 //   $('#cantidad').val(1);
                    auxcantidad.set(1);
                    auxprecio_unitario.set(data.precio);
                    $('#cantidad').focus();

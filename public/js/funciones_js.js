    ...
    // Para usar en las tablas  

    function formatoMoneda(value, row, index) {
        // Para usar data-formatter="formatoMoneda"      en los <th de las tablas
        
        if (value == null || isNaN(value)) return '<span class="text-muted">-</span>';
       // return '$' + Number(value).toLocaleString('es-AR', { minimumFractionDigits: cantDecMonto, maximumFractionDigits: cantDecMonto });
        return  Number(value).toLocaleString('es-AR', { minimumFractionDigits: cantDecMonto, maximumFractionDigits: cantDecMonto });
    }
   
    function formatoMoneda2Dec(value, row, index) {
        // Para usar data-formatter="formatoMoneda2Dec"      en los <th de las tablas
   ...
   function cantidadTotales(data) {
    ...
    function priceSorter(a, b) {
        // Utilizada por boostraptable columnas montos para ordenarlas correctamente, saca los puntos de miles y cambia la coma por punto para que se ordene como numero y no como string
        var aa = a.replaceAll('.', '')
        var bb = b.replaceAll('.', '')
         aa = aa.replace(',', '.')
         bb = bb.replace(',', '.') 

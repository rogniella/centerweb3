...
class cliente extends Model
{
    ...
    public function valida_delete_id($id)
    {
        ...
        $resultado = DB::select('SELECT count(Ot_Id) as cantidad FROM ot WHERE Ot_IdCLI = ?', [$id]);
        ...
        $resultado = DB::select('SELECT count(Comp_Id) as cantidad FROM comprobantes WHERE comp_IdCLI = ?', [$id]);
        if (isset($resultado[0]) and $resultado[0]->cantidad > 0) {
        ...
        $resultado = DB::select('SELECT count(MDes_IdProv) as cantidad FROM mcaja WHERE MDes_IdProv = ? * -1', [$id]);
        if (isset($resultado[0]) and $resultado[0]->cantidad > 0) {
    ...
    public function delete_id($id, $id_nuevo)
    {
        ...
        if ($id_nuevo != 0) {
            DB::update('UPDATE ot SET Ot_IdCli = ? WHERE Ot_IdCli = ?', [$id_nuevo, $id]);

            DB::update('UPDATE comprobantes SET comp_IdCli = ? WHERE comp_IdCli = ?', [$id_nuevo, $id]);

            DB::update('UPDATE mcaja SET MDes_IdProv = ? * -1 WHERE MDes_IdProv = ? * -1', [$id_nuevo, $id]);
        }

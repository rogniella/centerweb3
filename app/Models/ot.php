...
class ot extends Model
{
    ...
    public static function buscar($filtro_tipoinforme, $filtro_sucursal, $filtro_fecini = '', $filtro_fecfin = '', $filtro_tipoot = '', $filtro_estado = '')
    {
        ...
        if ($filtro_fecini != '') {
            ...
            $filter .= ' AND Ot_FecPedido >= ? and Ot_FecPedido <= ?';
    ...
    public function Descripcion_Estado($codigo)
    {
        ...
        $consulta = 'select descripcion from ot_estados where codigo = ?';

        if ($ret = DB::select($consulta, [$codigo])) {

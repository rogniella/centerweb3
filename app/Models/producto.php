...
class producto extends Model
{
    ...
    public static function reemplaza_codigo($familia, $id, $id_nuevo)
    {
        ...
        if ($familia === 'REC') {
            $resultado = DB::update(
                'UPDATE ot_ant SET otant_lejarmazon = ? WHERE otant_lejfamarmazon = ? AND otant_lejarmazon = ?',
                [$id_nuevo, $familia, $id]
            );
            displaylog($familia.' '.$id.' Arm Lejos:'.$resultado);

            $resultado = DB::update(
                'UPDATE ot_ant SET otant_cerarmazon = ? WHERE otant_cerarmazon = ?',
                [$id_nuevo, $id]
            );
        ...
        DB::update(
            'UPDATE moviproductos SET mov_idprod = ? WHERE mov_familia = ? AND mov_idprod = ?',
            [$id_nuevo, $familia, $id]
        );

        DB::delete(
            'DELETE FROM hisproductos WHERE hisprod_familia = ? AND hisprod_idprod = ?',
            [$familia, $id]
        );

        DB::delete(
            'DELETE FROM productos WHERE prod_familia = ? AND prod_id = ?',
            [$familia, $id]
        );
    ...
    public static function cambia_codigo($familia, $id, $id_nuevo)
    {
        ...
        displaylog($familia.' '.$id.' Arm Lejos');

        DB::update(
            'UPDATE ot_ant SET otant_cerarmazon = ? WHERE otant_cerarmazon = ?',
            [$id_nuevo, $id]
        );
        displaylog($familia.' '.$id.' Arm Cerca');

        DB::update(
            'UPDATE moviproductos SET mov_idprod = ? WHERE mov_familia = ? AND mov_idprod = ?',
            [$id_nuevo, $familia, $id]
        );
        displaylog($familia.' '.$id.' Mov');

        DB::update(
            'UPDATE hisproductos SET hisprod_idprod = ? WHERE hisprod_familia = ? AND hisprod_idprod = ?',
            [$id_nuevo, $familia, $id]
        );
        displaylog($familia.' '.$id.' Hisprod');

        DB::update(
            "UPDATE productos SET prod_id = ?, prod_codbarra = ?, prod_estado = 'A', Prod_UsuUltMan = 'WebCod' WHERE prod_familia = ? AND prod_id = ?",
            [$id_nuevo, $familia.$id_nuevo, $familia, $id]
        );
        displaylog($familia.' '.$id.' Prod');
    ...
    public static function listar_movimientos($familia, $idProd, $limite = 100)
    {
        ...
        $consulta = "SELECT * ,  CONCAT(mov_motivo,' ',mov_descripcion) as descripcion FROM moviproductos Where mov_familia = ? and mov_idProd = ? ORDER BY mov_idWEB DESC LIMIT ".(int)$limite;
    ...
    public static function listar_auditoria($familia, $idProd, $limite = 100)
    {
        ...
        $consulta = "SELECT * FROM hisproductos Where HisProd_familia = ? and HisProd_idProd = ? ORDER BY HisProd_idWEB DESC LIMIT ".(int)$limite;
        $ret = DB::select($consulta, [$familia, $idProd]);
    ...
    public static function listar($filtro_familia = '', $filtro_descripcion = '', $limite = 1000, $filtro_monto = '', $filtro_ultact = '', $filtro_estado = '', $filtro_marca = '')
    {
        ...
        $consulta = "SELECT  prod_idweb, prod_familia, prod_id,prod_descripcion,prod_categoria,prod_costo,prod_precio,prod_precio2
          ...
          , prod_fecultman , prod_usuultman ,  prod_usualta , prod_marca , UPPER(SUBSTRING_INDEX(Prod_Descripcion, ' ', 1)) marca FROM productos ".$filter." LIMIT ".(int)$limite;
    ...
    public function addMovimiento($operacion, $cantidad, $precioUnitario, $idOperacion, $idProv = 0, $detalle = '', $tipoOt = '', $sucursal = 0, $bonif = 0)
    {
    ...
    public function insertHistoria($campo, $valorant, $valornvo)
    {
        ...
        DB::insert(
            'INSERT INTO hisproductos (hisprod_familia, hisprod_idprod, hisprod_campo, hisprod_valorant, hisprod_valornvo, hisprod_usuario, hisprod_fecha, hisprod_sucursalorig)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            [
                $this->Prod_Familia,
                $this->Prod_Id,

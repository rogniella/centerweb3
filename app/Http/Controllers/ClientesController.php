...
use Illuminate\Support\Facades\Auth;  // Para usar SQL directamente (Raw SQL)
use Illuminate\Support\Facades\DB;  // Modelos a utilizar
...
class ClientesController extends Controller
{
    ...
    public function busca_autocompletar(Request $request)
    {
        ...
        $resbd = cliente::buscar(request()['terms'], '', 20);
    ...
    public function TemporalArreglo1buscar(Request $request)
    {
        ...
        foreach ($resbd as $elem) {
            ...
            if ($elem->cli_cuil == '00000000000' or $elem->cli_cuil == '0' or $elem->cli_cuil == '') {
                ...
                DB::update("UPDATE clientes SET Cli_CodRespIVA = 'CF' WHERE cli_Id = ?", [$elem->cli_id]);
                // lo paso a CF
            } else {
                if ($elem->cli_cuil != $elem->cli_documento) {
                    if (strlen($elem->cli_cuil) == 11) {
                        ...
                        DB::update("UPDATE clientes SET cli_coddocumento = 'CUIT', cli_documento = ? WHERE cli_Id = ?", [$elem->cli_cuil, $elem->cli_id]);
    ...
    public function update(Request $request)
    {
        ...
        if (! $registro->save()) {
            return response()->json(['error' => 'Error al guardar el cliente'], 500);
        }
    ...
    public function informecc_proceso()
    {
        ...
        $resbd = DB::select($consulta, $valores_where);
        ...
        foreach ($resbd as $objelem) {
            ...
            switch ($elem['MDes_Estado']) { // F=Tiene pago parcial    X=Todo Pago   D=Debe  Z= Anulado
                case 'F':
                    ...
                    if ($tipo_informe == 'P') {
                        $consultapago = 'SELECT sum(MPag_Monto) as pago FROM MPagos WHERE MPag_IdCompra = ?';
                        $row = DB::select($consultapago, [$elem['MCaj_Id']]);
                        $haber = $row[0]->pago;
                    }
                    break;
                case 'Z':

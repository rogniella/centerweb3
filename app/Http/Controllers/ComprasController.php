...
use App\Models\lote;
use App\Models\producto;  // Lotes de Compras
...
class ComprasController extends Controller
{
    ...
    public function AddItem(Request $request)
    {
        ...
        if ($request->ind_alta == 'CRI') {
            ...
            $valores = [
                'mlot_numlot' => $request->idcompra, 'mlot_sucursal' => $request->sucursal, 'mlot_fila' => $request->fila, 'mlot_familia' => 'CRI', 'mlot_idprod' => $request->idprod, 'mlot_cantidad' => $request->cantidad, 'mlot_precio' => $oproducto->Prod_Costo, 'mlot_descripcion' => '',
            ];

            compra::addItem($valores);

            // Retorna el IdCompra por si se genero
            return response()->json(['idcompra' => $request->idcompra, 'error' => '', 'descripcion' => $oproducto->Prod_Descripcion,
            ]);
            ...
            // Inserta 1 Item de la compra
            $valores = [
                'mlot_numlot' => $request->idcompra, 'mlot_sucursal' => $request->sucursal, 'mlot_fila' => $request->fila, 'mlot_familia' => $request->familia, 'mlot_idprod' => $request->idprod, 'mlot_cantidad' => $request->cantidad, 'mlot_precio' => $request->precio_lista, 'mlot_descripcion' => '',
            ];
    ...
    public function UpdateItem(Request $request)
    {
        ...
        $valores = [
            'mlot_numlot' => $request->idcompra, 'mlot_fila' => $request->fila, 'mlot_cantidad' => $request->cantidad, 'mlot_precio' => $request->precio_lista,
        ];

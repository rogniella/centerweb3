...
class CierresController extends Controller
{
    public function cierreCuenta()
    {
        $msgError = '';
        $cierre = new mcierre;
        $cierre->MCie_Moneda = request()['moneda'];
        $cierre->MCie_SaldoAnt = request()['saldo'];
        $cierre->MCie_UltId = request()['ultid'];
        $cierre->MCie_CodCta = request()['cuenta'];
        $cierre->MCie_Sucursal = request()['sucursal'];
    ...
    public function saldosCuentasDetalle2()
    {
        ...
        foreach ($results as $row) {
            ...
            $debe = '';
            $haber = '';
            if ($ultimoId < $row->MCaj_idWEB) {
                $ultimoId = $row->MCaj_idWEB;
            }
    ...
    private function descripcionOperacion($row, $accion)
    {
        ...
        if ($row->descri == 'falta') {
            if ($accion == 'E') {
                $descri = 'Se envío a';
                if ($row->MCaj_sucursalDes != $row->MCaj_sucursal) {
                    $descri .= ' Suc:'.$row->MCaj_sucursalDes;
                }
                $descri .= ' Cta:'.$row->MCaj_CtaDes;
                if ($row->MCaj_MonedaDes != $row->MCaj_Moneda) {
                    $descri .= ' Moneda:'.$row->MCaj_MonedaDes;
                }
            } else {
                $descri = 'Se recibío de';
                if ($row->MCaj_sucursalDes != $row->MCaj_sucursal) {
                    $descri .= ' Suc:'.$row->MCaj_sucursal;
                }
                $descri .= ' Cta:'.$row->MCaj_CtaOri;
                if ($row->MCaj_MonedaDes != $row->MCaj_Moneda) {
                    $descri .= ' Moneda:'.$row->MCaj_Moneda;
                }
            }
    ...
    private function calcularMonedas(string $sucursal): array
    {
        ...
        foreach ($monedas as $moneda) {
            ...
            foreach ($results as $row) {
                if ($row->MCaj_idWEB > $maxId) {
                    $maxId = $row->MCaj_idWEB;
                }
    ...
    {
        $sucursal = request()['sucursal'];
        $monedasData = json_decode(request()['monedas'], true);
        $cuenta = '01';
    ...
    public function listar()
    {
        ...
        $filtro = ' where ';
        if ($sucursal != '0') {
            $filtro = $filtro.' cie_sucursal = ? and';
            $valores[] = $sucursal;
        }

        $filtro = $filtro.' cie_cierre_fecha >= ? and cie_cierre_fecha <= ? order by cie_cierre_fecha desc, Cie_Id desc';
        $valores[] = $fecha;
        $valores[] = $fechafin;

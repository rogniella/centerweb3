...
class TarjetasController extends Controller
{
    ...
    private function proceso_linea_tarjeta($linea)
    {
        ...
        if ($tipoRegistro == '7') {
            ...
            $mov->mto_sindto = substr($linea, 75, 13); // Viene en cero no lo estamos usando
        ...
        if ($tipoRegistro == '8') {
            ...
            if ($mov->plazo_pago == 10) { // Es ahora 12
                ...
                displaylog('IdLiq:'.$idliquidacion.'Ahora 12 '.$mov->mto_otros_deb.' '.$mov->costo_financiero);
            ...
            if ($mov->plazo_pago == 2) { // En Cta y No ahora 12
                ...
                if ($mov->costo_financiero != 0) {
                    ...
                    if (round($mov->iva_anticipo, 0) == $iva) {
                        // Todo como 10%
                        displaylog('IdLiq:'.$idliquidacion.'NO promo 10,5 '.$mov->mto_otros_deb.' '.$mov->costo_financiero);
                        $mov->observacion = 'Sin Promo al 10.5%';
                    } else {
                        displaylog('IdLiq:'.$idliquidacion.'NO promo =='.$mov->mto_otros_deb.' '.$mov->costo_financiero);
                        $mov->observacion = 'Sin Promo 21% ';
                        $costo = round($mov->iva_anticipo / 0.21, 0);
                        if (round($mov->costo_financiero, 0) != $costo) {
                            displaylog('IdLiq:'.$idliquidacion.' Caso 10.5/21%  =='.$mov->costo_financiero.'  contra '.$costo);
                            $mov->observacion = 'VER Sin Promo 10.5/21% ';
                        }
                        // Tomo todo como iva 21
            ...
            try {
                ...
                if ($e->getCode() == '23000') {
                    flash("Registro duplicado en liquidaciones22! (idliquidacion: {$mov->idliquidacion})")->warning();
                } else {
                    throw $e;
                }
            }
        ...
        if ($tipoRegistro == '3') {
            ...
            $mov->mto_bruto = $mov->mto_bruto / 100;
            $mov->mto_sindto = substr($linea, 117, 13);  // Viene en cero no lo estamos usando
            $mov->mto_sindto = $mov->mto_sindto / 100;
            ...
            $tipoPlan = substr($linea, 153, 1); // tiene tabla de valores 0 Contado 1 Cuotas Tradic ...
            $mov->observacion = $tipoPlan.' ';
            ...
            if ($mov->cod_movimiento == '821') {
                // Es Contra Mov - Promo Banco Cts las Devoluciones
                $mov->observacion = $mov->observacion.'Devo.BcoCo';
            ...
            if ($mov->cod_movimiento == '892') {
                $mov->observacion = $mov->observacion.'Cargo';
                $mov->mto_bruto = $mov->mto_bruto * -1;
                $mov->mto_sindto = $mov->mto_sindto * -1;
                $mov->mto_final = $mov->mto_final * -1;
                $mov->mto_arancel = $mov->mto_arancel * -1;
            ...
            // *** FALTA Retenciones que no viene
...
*/
            try {
                ...
                if ($e->getCode() == '23000') {
                    flash("Duplicado en Operaciones Detalladas (idliquidacion: {$mov->idliquidacion})")->warning();
                } else {
                    throw $e;
                }
            }
    ...
    public function buscar_operaciones(Request $request)
    {
        ...
        $datos = tar_operacion::listar($request->filtro0, $request->filtro2, $request->fecha, $request->fechafin, $request->comercio, $request->terminal, $request->fechaope, $request->fechafinope, 1000);

        return response()->json(['results' => $datos]);

    } // Fin Buscar

    public function buscar_detalle_operacion(Request $request)
    {
        $datos = tar_operacion::buscarPorLoteYCupon($request->lote, $request->cupon);

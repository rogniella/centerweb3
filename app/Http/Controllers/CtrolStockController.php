...
use App\Models\lote;  // Para usar SQL directamente (Raw SQL)
...
class CtrolStockController extends Controller
{
    ...
    public function calcular_ajuste(Request $request)
    {
        ...
        $filtro = '';
        $valores = [$lote->Lot_Familia, $lote->Lot_Sucursal];
        if ($request->filtro != '') {
            $filtro = 'and Prod_Descripcion LIKE ?';
            $valores[] = '%'.$request->filtro.'%';
        } // If filtro
        ...
        $productos = DB::select($consulta, $valores);
    ...
    public function AddItem(Request $request)
    {
        ...
        $valores = [
            'mlot_numlot' => $request->idcompra, 'mlot_sucursal' => 0, 'mlot_fila' => $request->fila, 'mlot_familia' => $request->familia, 'mlot_idprod' => $request->idprod, 'mlot_cantidad' => $request->cantidad, 'mlot_precio' => 0, 'mlot_descripcion' => '',
        ];

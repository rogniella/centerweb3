...
class comprobante
{
    ...
    public static function find($suc, $tipoOT, $id)
    {
        ...
        try {
            ...
            foreach ($datos as $row) {
                ...
                if ($row->Mov_Operacion == 'V' or $row->Mov_Operacion == 'P') {
                    $row->Mov_Cantidad = $row->Mov_Cantidad * -1;
                }
    ...
    private function insert_comprobante($tipocomprobante)
    {
        ...
        try {
            if ($this->comp_sucursal == '') {
                $this->comp_sucursal = env('SUCURSAL_LOCAL');
            } // Sucursal + 099 Web
            if ($this->comp_idanula == '') {
                $this->comp_idanula = 0;
            }
            if ($this->comp_idfactura == '') {
                $this->comp_idfactura = 0;
            }
            if ($this->comp_idcli == '') {
                $this->comp_idcli = 0;
            }
